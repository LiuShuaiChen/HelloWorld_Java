...
while 1==1:
    ...
    if(respond.status_code != 200 and respond.status_code != 503):
        getCookies();
        respond = session.post(url, headers=headers, data=data);
    ...
    if (respond.status_code == 503):
        # 遇到ddos防火墙
        print >> log, str(respond.status_code) + time.strftime("%Y-%m-%d %H:%M:%S",time.localtime(time.time()));
        time.sleep(1000);
